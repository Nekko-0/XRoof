...
export default function ContractorDashboard() {
  ...
  // Lead source ROI
  const [leadRoi, setLeadRoi] = useState<{ name: string; jobs: number; won: number; revenue: number; avg_deal: number; conversion: number }[]>([])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Fetch lead source ROI (non-blocking)
          ...
          {leadRoi.length > 0 && (
            <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
              <p className="mb-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                Lead Source ROI <HelpTooltip text="Revenue generated by each lead source — shows which channels are most profitable" />
              </p>
                  ...
                  <tbody>
                    {leadRoi.map((s) => (
                      <tr key={s.name} className="border-b border-border/50">
                        <td className="py-2">
                          <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">{s.name}</span>
                        </td>
                        <td className="py-2 text-right font-medium text-foreground">{s.jobs}</td>
                        <td className="py-2 text-right font-medium text-emerald-500">{s.won}</td>
                        <td className="py-2 text-right font-medium text-foreground">${s.revenue.toLocaleString()}</td>
                        <td className="py-2 text-right text-muted-foreground">${s.avg_deal.toLocaleString()}</td>
                        <td className="py-2 text-right">
                          <span className={`font-medium ${s.conversion >= 50 ? "text-emerald-500" : s.conversion >= 25 ? "text-amber-500" : "text-muted-foreground"}`}>
                            {s.conversion}%
                          </span>
                        </td>
                      </tr>
